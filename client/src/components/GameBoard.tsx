...
export function GameBoard({ gameData, playerId, onSend }: Props) {
  ...
  return (
    <div className="flex-1 relative">
      ...
      {/* Minimal action overlay - only buttons */}
      <div className="absolute bottom-0 left-0 right-0 p-4 pointer-events-none">
        <div className="max-w-md mx-auto pointer-events-auto">

          {/* Alibi - just confirm button, cards are visible in 3D */}
          {phase === "alibi" && (
            <button
              onClick={() => onSend({ type: "pass_alibi" })}
              className="w-full py-4 bg-[var(--oink-teal)] text-white rounded-2xl font-black text-lg transition-all active:scale-[0.98] shadow-xl"
            >
              確認OK
            </button>
          )}
          ...
          {phase === "view_suspects" && isMyTurn && (
            <div className="bg-black/70 backdrop-blur rounded-2xl p-4 shadow-xl">
              <p className="text-white font-bold text-center text-sm mb-3">容疑者を2人選んで確認</p>
              ...
              <button
                ...
                className="w-full py-3 bg-[var(--oink-orange)] disabled:bg-white/10 disabled:text-white/30 text-white rounded-xl font-bold transition-all active:scale-[0.98]"
              >
          ...
          {phase === "view_suspects" && !isMyTurn && (
            <div className="bg-black/60 backdrop-blur rounded-2xl px-4 py-3 text-center">
              <p className="text-white text-sm font-bold">{players[currentTurn]?.name} が確認中...</p>
            </div>
          )}
          ...
          {phase === "tamper" && isDiscoverer && viewedSuspects && (
            <div className="bg-black/70 backdrop-blur rounded-2xl p-4 shadow-xl">
              <p className="text-white font-bold text-center text-sm mb-3">被害者と入れ替える？</p>
              <div className="space-y-2">
                ...
                <button
                  ...
                  className="w-full py-3 bg-white/20 text-white rounded-xl font-bold"
                >
                  しない
                </button>
              </div>
          ...
          {phase === "accuse" && isMyTurn && (
            <div className="bg-black/70 backdrop-blur rounded-2xl p-4 shadow-xl">
              <p className="text-white font-bold text-center text-sm mb-3">犯人はだれ？</p>
          ...
          {phase === "accuse" && !isMyTurn && (
            <div className="bg-black/60 backdrop-blur rounded-2xl px-4 py-3 text-center">
              <p className="text-white text-sm font-bold">{players[currentTurn]?.name} が告発中...</p>
            </div>
          )}
          ...
          {phase === "reveal" && (
            <button
              ...
              className="w-full py-4 bg-white text-[var(--oink-dark)] rounded-2xl font-black text-lg transition-all active:scale-[0.98] shadow-xl"
            >
              結果を見る
            </button>
          )}
          ...
          {(phase === "round_end" || phase === "game_over") && roundResult && (
            <div className="bg-black/80 backdrop-blur rounded-2xl p-4 shadow-xl">
                ...
                {roundResult.playersScore.map((p, i) => (
                  <div key={i} className="flex justify-between text-sm px-2">
                    ...
                    <span className="text-white/60">
                      ...
                      {" / "}
                      <span className="font-black text-white">{p.total}</span>
                    </span>
                  </div>
                ))}
              </div>
      ...
      <div className="absolute top-0 left-0 right-0 p-3 pointer-events-none">
        <div className="flex items-center justify-between max-w-md mx-auto">
          ...
          {isMyTurn && (phase === "view_suspects" || phase === "accuse" || phase === "tamper") && (
            <div className="bg-[var(--oink-orange)] rounded-full px-3 py-1.5 text-white text-xs font-black animate-pulse-soft pointer-events-none">
              YOUR TURN
            </div>
          )}
        </div>
